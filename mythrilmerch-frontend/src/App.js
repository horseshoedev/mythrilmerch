    ...
    const App = () => {
      ...
      const addToCart = async (productId) => {
        try {
          ...
          const data = await response.json();
          // Update cart state with the response from the backend (for this demo, it's the in-memory cart)
          // In a real app, you might re-fetch cart or update based on a more robust response
          // For this demo, we'll simply increment count if item exists, or add new
          setCart(prevCart => {
            ...
            if (existingItem) {
              ...
              return productToAdd ? [...prevCart, { productId: productId, quantity: 1, name: productToAdd.name, price: productToAdd.price }] : prevCart;
            }
          });
      ...
      return (
        <div className="min-h-screen bg-gray-100 font-sans text-gray-800 p-4 sm:p-6 lg:p-8 rounded-lg shadow-lg">
          {/* Header */}
          <header className="bg-white p-4 rounded-lg shadow-md mb-8 flex flex-col sm:flex-row justify-between items-center">
            <h1 className="text-3xl font-bold text-indigo-700 mb-4 sm:mb-0">E-commerce Demo</h1>
            {message && (
              <div className="bg-green-100 text-green-700 px-4 py-2 rounded-md transition-opacity duration-300 opacity-100">
                {message}
              </div>
            )}
          </header>
                    ...
                    {cart.map((item) => {
                      ...
                      return (
                        product && ( // Ensure product details are found before rendering
                          <li key={item.productId} className="flex justify-between items-center py-2 border-b border-gray-200 last:border-b-0">
